...
//
// OLD (broken) flow:
//   1. User applies points in UI
//   2. handlePay() calls create-order WITHOUT redeemedPoints
//      → Razorpay amount does NOT include loyalty discount
//   3. After payment, redeemAfterOrder(vd.orderId) calls POST /api/loyalty
//      with orderId = "FK-XXXX" (short display string)
//   4. /api/loyalty → redeemPoints({ orderId: "FK-XXXX" })
//   5. LoyaltyLedger.create({ order: "FK-XXXX" })
//      → Mongoose silently stores null because "FK-XXXX" is not an ObjectId
//   6. User.loyaltyPoints $inc DOES fire → balance goes down
//   7. But ledger entry has order: null → idempotency guard never works
//   8. Loyalty page shows nothing, double-deductions possible on retry
//
// NEW (fixed) flow:
//   1. User applies points in UI (redeemPts state)
//   2. handlePay() sends redeemedPoints to create-order
//   3. create-order validates server-side, returns validatedRedeemPoints
//      → Razorpay amount correctly includes the discount
//   4. verify receives validatedRedeemPoints, stores on Order document,
//      calls redeemPoints({ orderMongoId: order._id.toString() })
//      → Correct ObjectId → ledger entry created → idempotency works
//   5. NO separate /api/loyalty POST needed
...
function PayBtn({ loading, amount, disabled }: { loading: boolean; amount: number; disabled?: boolean }) {
    return (
...
export default function CheckoutPage() {
    ...
    const handlePay = async () => {
        ...
        // ── Step 1: create-order ──────────────────────────────────────────
        // Send redeemedPoints so server includes discount in Razorpay amount.
        // Server validates and echoes validatedRedeemPoints back.
        const cr = await fetch("/api/payment/create-order", {
            ...
            body:    JSON.stringify({
                ...
                // ← KEY FIX: pass points to create-order so discount is in Razorpay amount
        ...
        const rzp = new window.Razorpay({
            key:         cd.keyId,
            amount:      cd.amount * 100,  // server-confirmed amount
            currency:    "INR",
            name:        "FoodKnock",
            ...
            handler: async (response: unknown) => {
                ...
                // ── Step 2: verify ────────────────────────────────────────
                // Forward validatedRedeemPoints so verify stores it on Order
                // and calls redeemPoints() with the correct MongoDB _id.
                // NO separate /api/loyalty POST needed anymore.
                const vr = await fetch("/api/payment/verify", {
                    ...
                    body:    JSON.stringify(buildPayload({
                        ...
                        // ← KEY FIX: echo server-validated value, NOT raw redeemPts
                ...
                // ← NO redeemAfterOrder() call here — verify handles it internally
                clearCart();
                            ...
                            <div className="grid gap-5 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_400px]">
                                ...
                                <div className="flex flex-col gap-5">
                                    ...
                                    <Card>
                                        ...
                                        <div className="space-y-4 p-5">
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKInput value={form.customerName} onChange={set("customerName")} placeholder="Your full name" autoComplete="name" required disabled={isClosed} icon={<User size={14} strokeWidth={2} />} />
                                            </div>
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKInput value={form.phone} onChange={set("phone")} placeholder="10-digit mobile number" type="tel" autoComplete="tel" required disabled={isClosed} icon={<Phone size={14} strokeWidth={2} />} />
                                    ...
                                    <Card>
                                        ...
                                        <div className="space-y-4 p-5">
                                            ...
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKTextarea value={form.address} onChange={set("address")} placeholder="House no., Street, Colony, City..." rows={2} disabled={isClosed} icon={<MapPin size={14} strokeWidth={2} />} />
                                            </div>
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKInput value={form.landmark} onChange={set("landmark")} placeholder="Near school, temple, petrol pump..." disabled={isClosed} icon={<Tag size={14} strokeWidth={2} />} />
                                            </div>
                                        </div>
                                    </Card>
                                    ...
                                    <Card>
                                        ...
                                        <div className="p-5">
                                            <FKTextarea value={form.note} onChange={set("note")} placeholder="Spice level, no onions, extra sauce, gate code..." rows={3} disabled={isClosed} icon={<MessageSquare size={14} strokeWidth={2} />} />
                                        </div>
                                    </Card>
                                    ...
                                    <div className="block lg:hidden">
                                        <div className="rounded-3xl border border-orange-100 bg-white p-5 shadow-lg shadow-orange-100/40">
                                            ...
                                            <PayBtn loading={loading} amount={payableTotal} disabled={isClosed} />
                                            {!isClosed && <TrustNote />}
                                        </div>
                                    </div>
                                </div>
