...
//
// 1. Added <OrderProcessingOverlay> (full-screen blocking loader).
// 2. Added useOrderProcessing hook:
//      • startProcessing()   — called when user clicks Place/Pay
//      • stopProcessing()    — called on error (allows retry)
//      • finishProcessing()  — called on success: clears cart → redirects
// 3. Cart is NO LONGER cleared eagerly; it clears only inside finishProcessing.
// 4. Back-button is blocked while the overlay is visible (handled by the hook).
// 5. router.push() for success redirect moved inside finishProcessing.
// 6. All other UI, backend calls, and business logic are UNCHANGED.
...
import {
    ...
    Lock, Smartphone, BadgeCheck, Zap, AlertTriangle,
    Banknote, CreditCard, Wifi,
} from "lucide-react";
import {
    MIN_ORDER_AMOUNT, FREE_DELIVERY_AT, PLATFORM_FEE,
    FLAT_DELIVERY_FEE, calculateDeliveryFee, amountToFreeDelivery,
} from "@/lib/delivery";
...
function FKInput({ value, onChange, placeholder, type = "text", autoComplete, required, icon, disabled }: {
    value: string; onChange: (v: string) => void; placeholder: string;
    type?: string; autoComplete?: string; required?: boolean; icon?: React.ReactNode; disabled?: boolean;
}) {
    return (
        <div className="relative flex items-center">
            ...
            <input
                ...
                disabled={disabled}
                className={`w-full rounded-2xl border border-stone-200 bg-stone-50/60 py-3.5 text-sm font-medium text-stone-800 placeholder:text-stone-400 transition-all focus:border-orange-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-100 hover:border-stone-300 disabled:cursor-not-allowed disabled:opacity-60 ${icon ? "pl-10 pr-4" : "px-4"}`}
            />
        </div>
    );
}

function FKTextarea({ value, onChange, placeholder, rows = 3, icon, disabled }: {
    value: string; onChange: (v: string) => void; placeholder: string; rows?: number; icon?: React.ReactNode; disabled?: boolean;
}) {
    return (
...
function FirstDeliveryFreeBanner() {
    return (
        <div className="flex items-center gap-4 rounded-3xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-green-50 px-5 py-4 shadow-sm">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-emerald-200 bg-white shadow-sm">
                <Gift size={20} className="text-emerald-600" strokeWidth={2} />
            </div>
            <div>
                <p className="text-[13px] font-black text-emerald-800">🎁 Your first order includes FREE delivery!</p>
                <p className="mt-0.5 text-[11.5px] text-emerald-600">Delivery fee waived — a welcome gift from FoodKnock.</p>
            </div>
        </div>
    );
}
...
function ShopClosedBanner() {
    return (
        <div className="flex items-start gap-4 rounded-3xl border border-red-200 bg-gradient-to-r from-red-50 to-rose-50 px-5 py-5 shadow-sm">
...
function DeliveryBar({ subtotal, isFirstDeliveryFree }: { subtotal: number; isFirstDeliveryFree: boolean }) {
    if (isFirstDeliveryFree) {
        return (
            <div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                <Gift size={17} className="shrink-0 text-emerald-600" strokeWidth={2.5} />
                <div>
                    <p className="text-[12px] font-black text-emerald-800">🎁 First order — delivery is FREE!</p>
                    <p className="text-[11px] text-emerald-600">Welcome to FoodKnock 🎉</p>
                </div>
            </div>
        );
    }
...
function EmptyCart() {
    return (
        <div className="flex flex-col items-center rounded-3xl border border-stone-100 bg-white px-6 py-24 text-center shadow-sm">
            ...
            <p className="mt-2 max-w-xs text-sm leading-relaxed text-stone-500">Hunger hasn't knocked yet? Explore our menu and fill up your cart!</p>
...
function PaymentMethodSelector({
    selected, onChange, codAvailable, grandTotal, disabled,
}: {
    selected: PaymentMethod; onChange: (m: PaymentMethod) => void;
    codAvailable: boolean; grandTotal: number; disabled: boolean;
}) {
    return (
        <Card>
            <CardHead icon="💳" title="Payment Method" sub="How would you like to pay?" />
            <div className="space-y-3 p-5">
                <button
                    type="button" disabled={disabled} onClick={() => onChange("razorpay")}
                    className={`w-full rounded-2xl border-2 p-4 text-left transition-all ${
                        selected === "razorpay"
                            ? "border-orange-400 bg-orange-50/60 shadow-sm shadow-orange-100"
                            : "border-stone-200 bg-stone-50/50 hover:border-orange-200 hover:bg-orange-50/30"
                    } disabled:cursor-not-allowed disabled:opacity-60`}
                >
                    <div className="flex items-center gap-3">
                        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${selected === "razorpay" ? "bg-orange-500" : "bg-stone-200"}`}>
                            <Wifi size={16} className={selected === "razorpay" ? "text-white" : "text-stone-500"} strokeWidth={2.5} />
                        </div>
                        <div className="flex-1">
                            <div className="flex items-center gap-2">
                                <p className="text-[13px] font-black text-stone-900">Online Payment</p>
                                <span className="rounded-full bg-blue-100 px-2 py-0.5 text-[9px] font-black text-blue-700">RECOMMENDED</span>
                            </div>
                            <p className="text-[11px] text-stone-500">UPI · Cards · Net Banking · Wallets via Razorpay</p>
                        </div>
                        <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${selected === "razorpay" ? "border-orange-500 bg-orange-500" : "border-stone-300 bg-white"}`}>
                            {selected === "razorpay" && <div className="h-2 w-2 rounded-full bg-white" />}
                        </div>
                    </div>
                </button>

                <button
                    type="button" disabled={disabled || !codAvailable} onClick={() => codAvailable && onChange("cod")}
                    className={`w-full rounded-2xl border-2 p-4 text-left transition-all ${
                        !codAvailable
                            ? "cursor-not-allowed border-stone-100 bg-stone-50/30 opacity-60"
                            : selected === "cod"
                ...
                >
                    <div className="flex items-center gap-3">
                        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${selected === "cod" && codAvailable ? "bg-emerald-500" : "bg-stone-200"}`}>
                            <Banknote size={16} className={selected === "cod" && codAvailable ? "text-white" : "text-stone-500"} strokeWidth={2.5} />
                        </div>
                        <div className="flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                                <p className="text-[13px] font-black text-stone-900">Cash on Delivery</p>
                                {codAvailable ? (
                                    <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] font-black text-emerald-700">⚡ Available</span>
                                ) : (
                                    <span className="rounded-full bg-stone-100 px-2 py-0.5 text-[9px] font-black text-stone-500">Not available</span>
                                )}
                            </div>
                            {codAvailable ? (
                                <p className="text-[11px] text-stone-500">Pay cash when your order arrives</p>
                            ) : (
                                <p className="text-[11px] text-amber-600 font-semibold">COD available only for orders below ₹{COD_MAX_ORDER_AMOUNT}</p>
                            )}
                        </div>
                        <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${selected === "cod" && codAvailable ? "border-emerald-500 bg-emerald-500" : "border-stone-300 bg-white"}`}>
                            {selected === "cod" && codAvailable && <div className="h-2 w-2 rounded-full bg-white" />}
                        </div>
                    </div>
                </button>
                ...
                {codAvailable && (
                    <div className="flex items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50/60 px-3 py-2">
                        <Banknote size={12} className="shrink-0 text-emerald-600" />
                        <p className="text-[11px] text-emerald-700">
                            ⚡ COD available for orders below <strong>₹{COD_MAX_ORDER_AMOUNT}</strong> — pay cash at your door
                        </p>
                    </div>
                )}
            </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-stone-500">
                            {applied ? `−₹${rupees} saved on this order` : `${loyalty.balance} pts · worth ₹${(loyalty.balance * loyalty.pointValueInr).toFixed(0)}`}
                        </p>
                    </div>
                </div>
...
function PayBtn({
    loading, amount, disabled, isCod,
}: {
    loading: boolean; amount: number; disabled?: boolean; isCod?: boolean;
}) {
    return (
        <button type="submit" disabled={loading || disabled}
            className={`group relative flex w-full items-center justify-center gap-2.5 overflow-hidden rounded-2xl py-4 text-[15px] font-black text-white shadow-lg transition-all hover:brightness-105 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70 ${
                isCod
                    ? "bg-gradient-to-r from-emerald-500 via-emerald-500 to-teal-500 shadow-emerald-300/40 hover:shadow-emerald-300/60"
                    : "bg-gradient-to-r from-orange-500 via-orange-500 to-red-500 shadow-orange-300/40 hover:shadow-orange-300/60"
            }`}>
            ...
            {loading
                ...
                : isCod
                    ? <><Banknote size={15} strokeWidth={2.5} /> Place Order · Pay ₹{amount} on Delivery <ArrowRight size={15} className="ml-0.5 transition-transform group-hover:translate-x-0.5" /></>
...
function TrustNote({ isCod }: { isCod?: boolean }) {
    if (isCod) {
        return (
            <div className="mt-3 text-center">
                <p className="text-[11px] text-stone-400">💵 Pay cash when your rider arrives · No advance needed</p>
            </div>
        );
    }
    return (
        <div className="mt-3 space-y-1 text-center">
            <p className="text-[11px] text-stone-400">🔒 <span className="font-bold text-stone-500">Razorpay</span> secured · UPI · Cards · Net Banking · Wallets</p>
...
export default function CheckoutPage() {
    // router is still used for non-success navigations (auth redirect)
    ...
    // ── NEW: Order processing hook ────────────────────────────────────────────
    // "loading" is now derived from processingOrder; we keep no separate useState.
    const {
        processingOrder,
        ...
        stopProcessing,
        finishProcessing,
    } = useOrderProcessing({ clearCart });

    // Convenience alias — used in JSX where `loading` was previously used
    const loading = processingOrder;

    const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>("razorpay");

    const [shopOpen, setShopOpen]                       = useState<boolean | null>(null);
    const shopStatusFetched                             = useRef(false);
    const [isFirstDeliveryFree, setIsFirstDeliveryFree] = useState(false);
    const eligibilityFetched                            = useRef(false);
    ...
    const deliveryFee  = isFirstDeliveryFree ? 0 : calculateDeliveryFee(subtotal);
    const grandTotal   = subtotal + deliveryFee + PLATFORM_FEE;
    const payableTotal = Math.max(0, grandTotal - loyaltyDiscount);

    const codAvailable = grandTotal < COD_MAX_ORDER_AMOUNT;

    useEffect(() => {
        if (paymentMethod === "cod" && !codAvailable) {
            setPaymentMethod("razorpay");
            toast("Switched to online payment — order total exceeds COD limit.", {
                icon: "ℹ️",
                style: { background: "#fff7ed", color: "#9a3412", border: "1px solid #fed7aa", borderRadius: "12px" },
            });
        }
    ...
    useEffect(() => {
        if (shopStatusFetched.current) return;
        shopStatusFetched.current = true;
    ...
    useEffect(() => {
        if (eligibilityFetched.current) return;
        eligibilityFetched.current = true;
        fetch("/api/delivery/eligibility", { credentials: "include" })
            .then((r) => r.ok ? r.json() : { eligible: false })
            .then((d) => { if (d.eligible) setIsFirstDeliveryFree(true); })
            .catch(() => {});
    }, []);
            ...
            .then((d) => {
                if (d?.success && d.balance > 0) {
                    const info: LoyaltyInfo = {
                        balance:          d.balance,
                        pointValueInr:    d.pointValueInr    ?? 0.5,
                        minRedeem:        d.minRedeem        ?? 50,
                        maxRedemptionPct: d.maxRedemptionPct ?? 0.20,
                    };
                    setLoyalty(info);
    ...
    const validate = (): boolean => {
        if (shopOpen === false)                                   { toast.error("Shop is currently closed.");                           return false; }
        ...
        if (paymentMethod === "cod" && !codAvailable)             { toast.error(`COD only for orders below ₹${COD_MAX_ORDER_AMOUNT}`);  return false; }
        return true;
    };

    // ── COD flow ──────────────────────────────────────────────────────────────
    // CHANGED: removed clearCart() and router.push() — both live in finishProcessing()
    const handleCOD = async () => {
        const res  = await fetch("/api/orders/cod", {
            method:  "POST",
            headers: { "Content-Type": "application/json" },
            body:    JSON.stringify(buildPayload({ redeemedPoints: loyaltyApplied ? redeemPts : 0 })),
        });
        const data = await res.json();
        if (!res.ok || !data.success) {
            toast.error(data.message || "Failed to place COD order");
            stopProcessing(); // ← show overlay gone, allow retry
            return;
        }
        finishProcessing(data.orderId); // ← clears cart + redirects
    };

    // ── Razorpay flow ─────────────────────────────────────────────────────────
    // CHANGED: removed clearCart() and router.push() — both live in finishProcessing()
    // CHANGED: stopProcessing() replaces setLoading(false) on all error paths
    const handleRazorpay = async () => {
        const loaded = await loadRazorpay();
        if (!loaded) {
            toast.error("Payment gateway unavailable. Please retry.");
            stopProcessing();
            return;
        }
        ...
        const cr = await fetch("/api/payment/create-order", {
            ...
            body:    JSON.stringify({
                ...
                redeemedPoints: loyaltyApplied ? redeemPts : 0,
            }),
        });
        const cd = await cr.json();
        if (!cr.ok || !cd.success) {
            toast.error(cd.message || "Could not initialise payment");
            stopProcessing();
            return;
        }

        if (cd.isFirstDeliveryFreeApplied && !isFirstDeliveryFree) {
            setIsFirstDeliveryFree(true);
        }

        // Hide our overlay while Razorpay modal is open — Razorpay has its own UI
        // We'll re-show (via startProcessing) in the handler after payment succeeds
        stopProcessing();
        ...
        const rzp = new window.Razorpay({
            ...
            handler: async (response: unknown) => {
                // Payment captured — show overlay again while we verify + create order
                startProcessing("razorpay");

                const rp = response as {
                ...
                const vr = await fetch("/api/payment/verify", {
                    ...
                    body:    JSON.stringify(buildPayload({
                        ...
                        validatedRedeemPoints: cd.validatedRedeemPoints ?? 0,
                    })),
                });
                const vd = await vr.json();
                if (!vr.ok || !vd.success) {
                    toast.error(vd.message || "Payment verification failed");
                    stopProcessing();
                    return;
                }

                finishProcessing(vd.orderId); // ← clears cart + redirects
            },
            modal: {
                ondismiss: () => {
                    toast("Payment cancelled", { icon: "ℹ️" });
                    // overlay already hidden (stopProcessing was called before rzp.open())
                },
            },
        });
        rzp.open();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        startProcessing(paymentMethod === "cod" ? "cod" : "razorpay"); // ← show overlay
        ...
        try {
            ...
            stopProcessing(); // ← hide overlay on unexpected throw
        }
    };

    const isClosed = shopOpen === false;
    ...
    return (
        <>
            ...
            {/* ── Full-screen processing overlay (NEW) ── */}
                    ...
                    <div className="relative mx-auto max-w-6xl px-4 pb-7 pt-10 md:px-8">
                        ...
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <h1 className="text-3xl font-black tracking-tight text-stone-900 md:text-4xl">Almost there! 🍽️</h1>
                                {items.length > 0 && (
                        ...
                        <form onSubmit={handleSubmit} noValidate>
                            <div className="grid gap-5 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_400px]">
                                ...
                                <div className="flex flex-col gap-5">
                                    {isClosed && <ShopClosedBanner />}
                                    {!isClosed && isFirstDeliveryFree && <FirstDeliveryFreeBanner />}
                                    ...
                                    <Card>
                                        ...
                                        <div className="space-y-4 p-5">
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKInput value={form.customerName} onChange={set("customerName")} placeholder="Your full name" autoComplete="name" required disabled={isClosed || loading} icon={<User size={14} strokeWidth={2} />} />
                                            </div>
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKInput value={form.phone} onChange={set("phone")} placeholder="10-digit mobile number" type="tel" autoComplete="tel" required disabled={isClosed || loading} icon={<Phone size={14} strokeWidth={2} />} />
                                    ...
                                    <Card>
                                        ...
                                        <div className="space-y-4 p-5">
                                            <DeliveryBar subtotal={subtotal} isFirstDeliveryFree={isFirstDeliveryFree} />
                                            <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-2.5">
                                                <p className="text-[11px] font-medium text-blue-700">e.g. House 12, Shanti Nagar, Near Main Gate, Danta, Sikar 332702</p>
                                            </div>
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKTextarea value={form.address} onChange={set("address")} placeholder="House no., Street, Colony, City..." rows={2} disabled={isClosed || loading} icon={<MapPin size={14} strokeWidth={2} />} />
                                            </div>
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <FKInput value={form.landmark} onChange={set("landmark")} placeholder="Near school, temple, petrol pump..." disabled={isClosed || loading} icon={<Tag size={14} strokeWidth={2} />} />
                                            </div>
                                        </div>
                                    </Card>

                                    <PaymentMethodSelector
                                        selected={paymentMethod} onChange={setPaymentMethod}
                                        codAvailable={codAvailable} grandTotal={grandTotal}
                                    ...
                                    <Card>
                                        ...
                                        <div className="p-5">
                                            <FKTextarea value={form.note} onChange={set("note")} placeholder="Spice level, no onions, extra sauce, gate code..." rows={3} disabled={isClosed || loading} icon={<MessageSquare size={14} strokeWidth={2} />} />
                                        </div>
                                    </Card>
                                    ...
                                    <div className="block lg:hidden">
                                        <div className="rounded-3xl border border-orange-100 bg-white p-5 shadow-lg shadow-orange-100/40">
                                            <div className="mb-4">
                                                ...
                                                {isFirstDeliveryFree && <p className="text-[11px] font-black text-emerald-600">🎁 First order — delivery FREE!</p>}
                                            ...
                                            <PayBtn loading={loading} amount={payableTotal} disabled={isClosed} isCod={isCod} />
                                            {!isClosed && <TrustNote isCod={isCod} />}
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <div className="space-y-2.5 text-sm">
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-1.5">
                                                            ...
                                                            {deliveryFee === 0 && (
                                                                isFirstDeliveryFree
                                                                    ? <span className="flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[9px] font-black text-emerald-700"><Gift size={8} strokeWidth={3} /> FREE</span>
                                                    ...
                                                    {isFirstDeliveryFree && (
                                                        <div className="flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2">
                                                            <div className="flex items-center gap-1.5">
                                                                ...
                                                                <span className="font-black text-emerald-700">First Order Perk</span>
                                                            </div>
                                                            <span className="font-black text-emerald-700">−₹{FLAT_DELIVERY_FEE}</span>
                                                        </div>
                                                    )}
                                                    {loyaltyApplied && (
                                                    ...
                                                    <div className={`flex items-center justify-between rounded-xl border px-3 py-2 ${isCod ? "border-emerald-200 bg-emerald-50" : "border-blue-100 bg-blue-50"}`}>
                                                        <div className="flex items-center gap-1.5">
                                                            {isCod
                                                                ? <Banknote size={11} className="text-emerald-600" />
                                                                : <CreditCard size={11} className="text-blue-600" />
                                                            }
                                                            <span className={`text-[11px] font-black ${isCod ? "text-emerald-700" : "text-blue-700"}`}>
                                                                {isCod ? "Cash on Delivery" : "Online Payment"}
                                                            </span>
                                                        </div>
                                                        <span className={`text-[10px] font-bold ${isCod ? "text-emerald-600" : "text-blue-600"}`}>
                                                            {isCod ? "Pay at door" : "Pay now"}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div className="my-4 h-px bg-stone-100" />
