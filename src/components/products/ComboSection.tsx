...
// WAVE 3C FINAL — PURE RENDERER (unchanged from WAVE 3B, confirmed clean)
// CONFIRMED:
//   • Receives pre-filtered `combos` array from MenuClient — zero internal filtering
//   • No outer wrapper section in MenuClient — ComboSection IS the section (owns dark bg, headings, paddings)
//   • COMBO_ANIMATION_CSS is module-level constant — not recreated on re-render
//   • All Wave 2 UX touch fixes preserved (touchAction, pointer-events-none)
//   • getBadge is pure (index mod) — no allocation, no memo needed
//   • getItemsLine is a pure function — called inline
//   • ComboCard: memoized, useCallback on handleAdd
...
// PERF: module-level constant — allocated once, never recreated
const COMBO_ANIMATION_CSS = `
...
// ── Individual Combo Card — memoized ──────────────────────────────────────
const ComboCard = memo(function ComboCard({
    ...
    return (
        <Link href={`/menu/${product.slug}`}
            className={`fk-combo-card group relative flex flex-col overflow-hidden rounded-2xl bg-white transition-all duration-300 ${unavailable ? "opacity-60" : "hover:-translate-y-1 hover:shadow-2xl hover:shadow-orange-200/50"
                }`}
            style={{
            ...
            {/* Image — PERF: cdnImage at 480px for combo cards */}
            <div className="relative h-40 w-full overflow-hidden bg-stone-100">
                <img
                    ...
                    decoding="async"
                    className={`h-full w-full object-cover transition-transform duration-500 ${unavailable ? "grayscale-[40%]" : "group-hover:scale-[1.08]"
                        }`}
                ...
                {!unavailable && (
                    <div className="absolute left-2.5 top-2.5 flex items-center gap-1 rounded-full px-2.5 py-1 text-[9.5px] font-black text-white shadow-lg pointer-events-none"
            ...
            <div className="flex flex-1 flex-col p-3.5">
                ...
                <button
                    ...
                    className={`w-full rounded-xl py-2.5 text-[12px] font-black transition-all duration-200 ${unavailable ? "cursor-not-allowed bg-stone-100 text-stone-400" : "text-white active:scale-95"
                        }`}>
...
// ── Section — memoized ─────────────────────────────────────────────────────
// WAVE 3C: This component IS the complete combo section including:
//   • Dark themed background, decorative blobs, accent line
//   • Section heading, "Today's Deals" label, scroll controls
//   • Savings teaser strip
//   • Horizontal scrolling card list + view-all tile
//   • Mobile view-all button
// MenuClient renders this DIRECTLY with no outer wrapper — no double nesting.
const ComboSection = memo(function ComboSection({
    combos,
    onViewAll,
}: {
    combos: Product[];
    ...
    const scrollLeft = useCallback(() => ref.current?.scrollBy({ left: -280, behavior: "smooth" }), []);
    const scrollRight = useCallback(() => ref.current?.scrollBy({ left: 280, behavior: "smooth" }), []);
        ...
        <section
            ...
            <div className="relative mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-10">
                ...
                <div ref={ref}
                    ...
                    {combos.map((combo, i) => (
                        <div key={combo._id} className="shrink-0 snap-start" style={{ touchAction: "pan-x pan-y" }}>
                    ...
                    {/* View-all tile */}
                    <div
                        className="shrink-0 snap-start flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/5 px-5 text-center transition-all hover:border-orange-500/30 hover:bg-orange-500/8"
                        style={{ minWidth: 126, touchAction: "auto" }}
                        onClick={onViewAll}>
            ...
            {/* PERF: module-level constant — not recreated on re-render */}
            <style>{COMBO_ANIMATION_CSS}</style>
        </section>
    );
});
