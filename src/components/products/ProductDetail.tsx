...
// PERF:
//   - All product/upsell images: native <img> + cdnImage() — zero Vercel transformations
...
const TAG_STYLES: Record<string, { bg: string; text: string; icon: React.ReactNode }> = {
    ...
    "best seller": {
        ...
        icon: <BadgeCheck size={11} strokeWidth={2.5} />,
    },
    "must try": {
...
// ─── Upsell mini-card ─────────────────────────────────────────────────────
const UpsellCard = memo(function UpsellCard({ product }: { product: Product }) {
    ...
    const handleAdd = useCallback(
        (e: React.MouseEvent) => {
            ...
            toast.success(`${product.name} added to your order! 🛒`, TOAST_STYLE);
            setTimeout(() => setAdding(false), 1500);
        },
        [isUnavailable, addItem, product]
    );
        ...
        <div className="group flex overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-sm transition-all duration-200 hover:border-orange-200 hover:shadow-md hover:shadow-orange-100/60">
            ...
            <div className="flex flex-1 flex-col justify-between p-3">
                <div>
                    ...
                    <p className="line-clamp-1 text-[13px] font-black text-stone-800">
                        {product.name}
                    </p>
...
// ─── Info card data ────────────────────────────────────────────────────────
const INFO_CARDS_STATIC = [
    { icon: <Clock size={16} className="text-amber-500" strokeWidth={2} />, label: "Prep Time", staticValue: "15–20 min" },
    { icon: <Users size={16} className="text-orange-500" strokeWidth={2} />, label: "Serves", staticValue: "1 Person" },
];
...
export default function ProductDetail({
    ...
    const { isUnavailable, rating, ratingCount } = useMemo(
        () => ({
            isUnavailable: !product.isAvailable || product.stock <= 0,
            rating: product.rating ?? 4.2,
            ratingCount: Math.floor((parseInt(product._id.slice(-4), 16) % 800) + 100),
        }),
        [product]
    );

    const stockLabel = useMemo(
        () => (product.stock > 0 ? `${Math.min(product.stock, 99)}+ left` : "Sold Out"),
        [product.stock]
    );
    ...
    const handleAddToCart = useCallback(() => {
        ...
        toast.success(
            `${qty > 1 ? `${qty}× ` : ""}${product.name} added to your cart! 🛒`,
            TOAST_STYLE
        );
        setTimeout(() => setAdding(false), 600);
    }, [isUnavailable, qty, addItem, product]);
                ...
                <div className="grid gap-8 md:grid-cols-2 md:gap-14">
                    ...
                    <div className="relative">
                        <div className="relative aspect-square overflow-hidden rounded-3xl bg-gradient-to-br from-amber-50 to-orange-100 shadow-xl shadow-orange-100/70">
                            {/* PERF: native img + cdnImage, 700px for hero LCP image */}
                            <img
                                src={cdnImage(product.image, 700)}
                                alt={product.name}
                                loading="eager"
                    ...
                    <div className="flex flex-col pt-6 md:pt-0">
                            ...
                            <div className="mb-4 flex flex-wrap gap-1.5">
                                {product.tags.map((tag) => {
                                    ...
                                    return (
                                        <span
                                            key={tag}
                                            className={`flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-bold capitalize ${s.bg} ${s.text}`}
                                        >
                                            {s.icon}
                                            {tag}
                                        </span>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-3">
                            ...
                            <span className="text-xs font-medium text-stone-400">
                                {ratingCount.toLocaleString()} happy customers
                            </span>
                            <div className="h-3 w-px bg-stone-200" />
                            <span
                                className={`flex items-center gap-1.5 text-xs font-bold ${isUnavailable ? "text-red-500" : "text-green-600"
                                    }`}
                            >
                                <span
                                    className={`h-1.5 w-1.5 rounded-full ${isUnavailable ? "bg-red-400" : "bg-green-500 animate-pulse"
                                        }`}
                                />
                        ...
                        <div className="mt-5 flex items-end gap-2">
                            <span className="text-4xl font-black text-orange-600 md:text-5xl">
                                ₹{product.price}
                            </span>
                            <span className="mb-1.5 text-sm font-medium text-stone-400">
                                per serving
                            </span>
                        </div>

                        {product.shortDescription && (
                            <p className="mt-1.5 text-sm font-semibold text-stone-500 leading-relaxed">
                                {product.shortDescription}
                            </p>
                        )}
                        ...
                        <div className="mb-5 grid grid-cols-3 gap-3">
                            {INFO_CARDS_STATIC.map((card) => (
                                <div
                                    key={card.label}
                                    className="flex flex-col items-center gap-1.5 rounded-2xl border border-amber-100 bg-white p-3 text-center shadow-sm"
                                >
                                    {card.icon}
                            ...
                            {/* Stock card */}
                            <div className="flex flex-col items-center gap-1.5 rounded-2xl border border-amber-100 bg-white p-3 text-center shadow-sm">
                                <Package size={16} className="text-green-500" strokeWidth={2} />
                                <span className="text-xs font-black text-stone-800">
                                    {stockLabel}
                                </span>
                                <span className="text-[10px] text-stone-400">Availability</span>
                            </div>
                        </div>
                        ...
                        {product.ingredients && product.ingredients.length > 0 && (
                            <div className="mb-5">
                                <p className="mb-2.5 text-xs font-black uppercase tracking-wider text-stone-500">
                                    What&apos;s Inside
                                </p>
                                <div className="flex flex-wrap gap-1.5">
                                    {product.ingredients.map((ing) => (
                                        <span
                                            key={ing}
                                            className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-stone-600 capitalize"
                                        >
                                            {ing}
                                        </span>
                                    ))}
                            ...
                            >
                                {adding ? (
                                    ...
                                    <><ShoppingCart size={16} strokeWidth={2.5} /> Add{" "}
                                        {qty > 1 ? `${qty} × ` : ""}to Cart · ₹{product.price * qty}</>
                                )}
                            </button>
                        </div>
                        ...
                        <div className="mb-5 flex items-center gap-3">
                            <div>
                                ...
                                <p className="text-xs text-stone-400 mt-0.5">
                                    Customers who order this also love these
                                </p>
                            </div>
                    ...
                    <section className="mt-16 pb-32 md:pb-12">
                        <div className="mb-5 flex items-center gap-3">
                            <div>
                                ...
                                <p className="text-xs text-stone-400 mt-0.5">
                                    More from {normalizeCategory(product.category)}
                                </p>
                            </div>
                            <div className="h-px flex-1 bg-gradient-to-r from-amber-200 to-transparent" />
                            <Link
